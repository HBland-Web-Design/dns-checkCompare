...
def fill_domain_template(domain, results):
    # Load the HTML template
    ...
    for record in results:
        if record['status'] == "Fail":
            RED += 1
            row = f"\n<tr><td style='border: 1px solid black; padding: 8px; text-align: left;'>{record['record']}</td><td style='border: 1px solid black; padding: 8px; text-align: left;'>{record['recordType']}</td><td style='border: 1px solid black; padding: 8px; text-align: left;'>{record['status']}</td><td style='border: 1px solid black; padding: 8px; text-align: left;'>{record['msg']}</td></tr>"
        elif record['status'] == "Pass":
            RED += 0
            row = f"\n<tr><td style='border: 1px solid black; padding: 8px; text-align: left;'>{record['record']}</td><td style='border: 1px solid black; padding: 8px; text-align: left;'>{record['recordType']}</td><td style='border: 1px solid black; padding: 8px; text-align: left;'>{record['status']}</td><td style='border: 1px solid black; padding: 8px; text-align: left; background-color: lightgrey'></td></tr>"
        rows_content += row
